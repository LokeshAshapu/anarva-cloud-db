...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-64 border-r border-slate-800 bg-slate-900/50 p-4 flex-col justify-between shrink-0">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 text-xs font-semibold rounded-xl transition ${
                isActive
      ...
      <div className="p-3 bg-slate-950 border border-slate-800/80 rounded-xl text-xs text-slate-400 space-y-1 mt-4">
