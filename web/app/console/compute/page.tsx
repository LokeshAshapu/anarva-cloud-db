...
export default function ComputeEnginePage() {
  ...
  const fetchInstances = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/compute/instances?projectId=proj-default`, { headers: authHeaders })
        .then((r) => r.json())
        .catch(() => null)
  ...
  const handleCreateInstance = async () => {
    ...
    try {
      ...
      const authHeaders = getAuthHeaders()
      const res = await fetch(`${API_BASE_URL}/api/v1/compute/instances`, {
        method: 'POST',
      ...
      if (attachVolume) {
        setVolumes([
          ...volumes,
          { id: `vol-${Date.now()}`, name: `vol-${instName}-data`, sizeGb: volumeSizeGb, type: 'NVME_SSD', status: 'ATTACHED', instanceId: instName },
        ])
      }
    } finally {
  ...
  const handleDeleteInstance = async (id: string, instName: string) => {
    if (confirm(`Are you sure you want to terminate compute instance '${instName}'?`)) {
      const authHeaders = getAuthHeaders()
      await fetch(`${API_BASE_URL}/api/v1/compute/instances/${id}`, {
        method: 'DELETE',
        headers: authHeaders,
      }).catch(() => null)
  ...
  const handleStopInstance = async (id: string) => {
    const authHeaders = getAuthHeaders()
    await fetch(`${API_BASE_URL}/api/v1/compute/instances/${id}/stop`, {
      method: 'POST',
      headers: authHeaders,
    }).catch(() => null)
  ...
  const handleStartInstance = async (id: string) => {
    const authHeaders = getAuthHeaders()
    await fetch(`${API_BASE_URL}/api/v1/compute/instances/${id}/start`, {
      method: 'POST',
      headers: authHeaders,
    }).catch(() => null)
